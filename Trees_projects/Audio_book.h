...
Audio_book_Tree *create_audiobook_tree()
{
    ...
    while (fscanf(fp, " %s", word) == 1)
    {
        ...
        temp->right = NULL;
        temp->height = 0;
        // Calling the function to insert this new Audiobook into the AVL tree
...
Audio_book_user_Tree *Leftrotation_abu(Audio_book_user_Tree *root)
{
    ...
    int left = root->left ? root->left->height : -1;
    int right = root->right ? root->right->height : -1;
    int h = maxi(left, right);// Modifying the height 
    root->height = h + 1;
    int tleft = temp->left ? temp->left->height : -1;
    int tright = temp->right ? temp->right->height : -1;
    int th = maxi(tleft, tright); // Modifying the height 
    temp->height = th + 1;
...
void add_AudioBook(Audio_book_Tree **abt)
{
    int id; 
    printf("Enter the id for creating New Audio Book");
    scanf("%d",&id);
    ...
    if (ptr == NULL)
    {
        ...
        printf("\n Enter the Title for New Audio Book ");
        scanf("%s",title);
        printf("\n Enter the Author name  for New Audio Book ");
        scanf("%s",author);
        printf("\n Enter the Narrator name for New Audio Book ");
        scanf("%s",narrator);
...
void Edit_audiobook(Audio_book_Tree **abbt)
{
    Audio_book_Tree* abt=(*abbt);
...
void Add_to_User_library(User_Tree ** ul, Audio_book_Tree **al)
{
    User_Tree *userlist=(*ul);
    Audio_book_Tree *audiolist=(*al);
    int n;
    int id;
...
void Search_audiobook(Audio_book_Tree **abbt)
{
    Audio_book_Tree *abt=(*abbt);
    int n;
    char word[MAXLEN];
...
void Listening_progress(User_Tree **u, Audio_book_Tree **abbt)
{
    User_Tree *users=(*u);
     Audio_book_Tree *abt=(*abbt);
    int id;
...
void Display_user_libraries(User_Tree **u)
{
    User_Tree* users= (*u);
    int id;
...
void Rate_Audiobook(User_Tree **u, Audio_book_Tree **abbt)
{
    User_Tree *users= (*u);
     Audio_book_Tree *abt= (*abbt);
    int id;
...
void Most_Popular_Audio_book(Audio_book_Tree ** head)
{
    Audio_book_Tree* root=(*head);
    Audio_book_Tree *ans = NULL;
    float max = 0.0;
...
void Listening_History_report_User(User_Tree **head)
{
    User_Tree*root=(head);
    int id;
    printf("Enter the ID of Audio Book");
    scanf("%d",&id);
...
void Listening_History_Audio_book(Audio_book_Tree **r,  User_Tree ** ur)
{
    Audio_book_Tree *root=(*r);
      User_Tree *user_root=(*ur);
    int id;
    printf("Enter the ID of Audio Book");
    scanf("%d",&id);
...
void Range_search(Audio_book_Tree **head)
{

    Audio_book_Tree* root=(*head);
    int id1,id2;
    printf("Enter the ID of first Audio Book : ");
    scanf("%d",&id1);
     printf("Enter the ID of second Audio Book : ");
    scanf("%d",&id2);
